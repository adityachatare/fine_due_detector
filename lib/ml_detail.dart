                      ...
                      if (snapshot.hasData) {
                        return Container(
                            foregroundDecoration: (widget._scannerType ==
                                    TEXT_SCANNER) ? TextDetectDecoration(_currentTextLabels, snapshot.data) : (widget._scannerType == FACE_SCANNER)
...
child: Column(
    ...
    RaisedButton(
      ...
      onPressed: (){
        Navigator.of(context).push(MaterialPageRoute(
          builder: (context) => Vars(value: text,),
        ));
      }
      ),
  ...
  Future<Size> _getImageSize(Image image) {
    ...
    image.image.resolve(ImageConfiguration()).addListener(new ImageStreamListener((ImageInfo info,bool synchronousCall)
    { 
...
class _LabelDetectPainter extends BoxPainter {
  //final List<VisionLabel> _labels;
  //final Size _originalImageSize;
 // _LabelDetectPainter(labels, originalImageSize): _labels = labels;
